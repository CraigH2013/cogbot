...
gotRejected.stems([
  ...combos(
    `I did not get ${combos('selected', 'approved', 'promoted', 'accepted')}`
  ),
...
triedAndDidNotSucceed.stems([
  ...combos(
    `${combos('I tried but', 'Even though I tried I')} ${combos(
      'did not succeed',
      'failed',
      'did not make it'
    )}`
...
havingRelationshipIssues.stems([
  ...combos(
    `I am having ${combos('issues', 'problems', 'struggles')} in my ${combos(
      'marriage',
      'relationship'
    )}`
  ),
